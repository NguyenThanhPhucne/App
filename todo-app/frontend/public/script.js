    ...
    const mockTasks = [
        {
            _id: '1',
            task_id: 1,
            description: 'Complete project documentation and review with team',
        ...
        },
        {
            _id: '2',
            task_id: 2,
            description: 'Review code changes and merge pull requests',
            due_date: new Date(Date.now() - 86400000), // Yesterday (overdue)
            status: 'pending',
            tags: ['review', 'development', 'git']
        },
        {
            _id: '3',
            task_id: 3,
            description: 'Team meeting preparation and agenda setup',
        ...
        },
        {
            _id: '4',
            task_id: 4,
            description: 'Update database schema and run migrations',
        ...
        },
        {
            _id: '5',
            task_id: 5,
            description: 'Design new user interface mockups',
            due_date: new Date(Date.now() - 172800000), // 2 days ago (overdue)
            status: 'pending',
...
async function addTask(event) {
    ...
    const task = {
        ...
        task_id: parseInt(elements.taskIdInput.value),
    ...
    try {
        ...
        clearErrors();
        showToast('Task created successfully! Ready to boost your productivity.', 'success');
...
function validateForm() {
    ...
    // Validate task ID
    const taskId = parseInt(elements.taskIdInput.value);
    if (!taskId || taskId <= 0) {
    ...
    return [...taskList].sort((a, b) => {
        switch (currentSort) {
            ...
            case 'priority':
                ...
                const bOverdue = isTaskOverdue(b) && a.status === 'pending';
...
function clearCompletedTasks() {
    ...
    showConfirmDialog(
        ...
        `Are you sure you want to delete ${completedCount} completed task(s)? This action cannot be undone.`
    ).then(confirmed => {
        if (confirmed) {
            ...
            renderTasks();
            updateStats();
            showToast(`Cleared ${completedCount} completed task(s) successfully!`, 'success');
        }
    });
}
...
function createListTaskElement(task) {
    ...
    const overdueIndicator = isOverdue && task.status === 'pending' 
...
function toggleStatus(id, currentStatus) {
    ...
    if (taskIndex !== -1) {
        ...
        renderTasks();
        updateStats();
        showToast(`Task ${tasks[taskIndex].status === 'completed' ? 'completed' : 'reopened'} successfully!`, 'success');
    }
}
...
function deleteTask(id) {
    ...
    showConfirmDialog('Are you sure you want to delete this task?', 'This action cannot be undone and will permanently remove the task.')
        .then(confirmed => {
            if (confirmed) {
                const taskIndex = tasks.findIndex(task => task._id === id);
                if (taskIndex !== -1) {
